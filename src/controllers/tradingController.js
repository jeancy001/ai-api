...
export async function start(req, res) {
  ...
  /**
   * Persist permission before starting the engine.
   */
  settings.autoTradingEnabled = true;
  ...
  try {
     ...
     * Never change emergencyStop or authorization here.
     */
...
export async function stop(req, res) {
  ...
  /**
   * Normal Stop is different from Emergency Stop.
   * It can report that there is nothing currently running.
   */
  if (
    settings.autoTradingEnabled !== true &&
    engineRunning !== true
...
/* ============================================================
   ...
   IMPORTANT:
   Emergency Stop is NOT a normal Stop request.

   It always activates the persistent safety lock, even when
   auto-trading is already stopped.
============================================================ */
...
export async function emergency(req, res) {
  ...
  /**
   * Persist the safety lock first.
   *
   * This endpoint is intentionally idempotent.
   * Emergency Stop must never fail simply because the trading
  ...
  /**
   * Stop the local scheduler after the database safety state
   * has already blocked future trade execution.
   */
  const engineStopped =
    await stopEngineSafely(
      userId,
...
/* ============================================================
   ...
   Releasing Emergency Stop removes ONLY the emergency lock.

   IMPORTANT:
   - Does NOT authorize real-money trading.
   - Does NOT start the trading engine.
   - Does NOT enable auto-trading.
   - User must explicitly press Start afterwards.
============================================================ */
...
export async function releaseEmergencyStop(req, res) {
  ...
  if (settings.emergencyStop !== true) {
    return res.status(200).json({
      success: true,
      message:
        "Emergency Stop is already inactive. Auto-trading remains stopped.",
      data: serializeSettings(settings),
    });
  }
   ...
   * Releasing the emergency lock MUST NEVER restart trading.
   */
  settings.autoTradingEnabled = false;
  ...
  /**
   * Defensive safety check: make sure a stale local scheduler
   * cannot continue running after the lock is released.
   *
   * The engine remains stopped. The user must explicitly start it.
   */
  const engineStopped =
    await stopEngineSafely(
      userId,
